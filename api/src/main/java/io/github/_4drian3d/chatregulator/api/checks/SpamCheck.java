...
public final class SpamCheck implements Check {
  ...
  public @NotNull CheckResult check(final @NotNull InfractionPlayer player, final @NotNull String string) {
    ...
    // Previous Executed Messages/Commands Check
    ...
    // Last Message/Command Check
    if (this.checkSimilarity(string, chain.last())) {
  ...
  private boolean checkSimilarity(final @NotNull String actual, final @NotNull String last) {
    if (this.normalizationConfig.enabled()) {
      final String lastNormalized = Replacer.NORMALIZATION_PATTERN.matcher(
          Normalizer.normalize(
              last.toLowerCase(Locale.ROOT),
              this.normalizationConfig.normalization()
          )).replaceAll("");
      final String actualNormalized = Replacer.NORMALIZATION_PATTERN.matcher(
          Normalizer.normalize(
              actual.toLowerCase(Locale.ROOT),
              this.normalizationConfig.normalization()
          )).replaceAll("");
      return actualNormalized.equals(lastNormalized);
    } else {
      return actual.equalsIgnoreCase(last);
    }
  }
